...
Built with resilience at its core, Lumina VE automatically scales its feature set based on container memory limits and environment toggles. When deep learning is disabled or memory is constrained, it seamlessly falls back to lightweight, non-ML heuristic algorithms to maintain 100% schema population without the memory penalty. It delivers sub-second insights on micro-tier instances while providing deep machine-learning analysis when hardware permits.
...
* **[OpenCV](https://opencv.org/):** Core computer vision engine used for Laplacian variance blur evaluation, spatial dimension calculations, Canny edge-detection fallbacks, and K-Means color matrix clustering.
...
# Set to false to use lightweight non-ML fallbacks (Canny edge-detection & quantization)
...
```bash
curl -X POST [http://127.0.0.1:8000/v1/vision/validate/url](http://127.0.0.1:8000/v1/vision/validate/url) \
  -H "X-API-Key: dev_lumina_secure_key_2026" \
  -H "Content-Type: application/json" \
  -d '{
    "url": "[https://raw.githubusercontent.com/python-pillow/Pillow/main/Tests/images/hopper.png](https://raw.githubusercontent.com/python-pillow/Pillow/main/Tests/images/hopper.png)"
  }'
```
...
* **SSRF Protection & TOCTOU DNS Rebinding Defense:** To prevent Server-Side Request Forgery (SSRF) and Time-of-Check to Time-of-Use (TOCTOU) DNS rebinding attacks, the dependency injection layer resolves the hostname to an IP address asynchronously and checks it against private, loopback, and link-local CIDR blocks. The subsequent transport step streams directly from the validated IP while retaining the original Host header. Furthermore, the transport layer natively overrides the TLS SNI (Server Name Indication) extension during the handshake to perfectly mimic a browser, preventing `421 Misdirected Request` or `403 Forbidden` errors on shared hosts and CDNs.
* **Lazy-Loaded Neural Runtimes & Fallbacks:** Machine learning frameworks like ONNX and `rembg` consume significant RAM upon import. Lumina VE defers the importing of `rembg` until runtime execution. If heavy ML operations are disabled or RAM is constrained, the dependencies are never loaded into memory, preserving a lightweight baseline footprint (~60MB - 90MB). Instead of simply dropping the analysis, the engine immediately pivots to OpenCV Canny edge-detection pipelines and NumPy color quantization. This guarantees that all metrics (padding, color hexes, background ratios) are still calculated with a negligible RAM footprint.
* **Container-Aware Memory Safeguards (cgroups v1/v2):** Before initiating background segmentation, the engine queries operating system cgroups memory drivers (`/sys/fs/cgroup/memory.max` and `memory.limit_in_bytes`). If available RAM drops below `MIN_RAM_REQUIRED_MB` (e.g., 400MB), the engine dynamically downgrades from the heavy ML steps to prevent container Out-Of-Memory (OOM) crashes.
* **Explicit System Warning Lifecycle:** Rather than failing silently or raising generic 500 errors when features are disabled or degraded, Lumina VE captures operational warnings into a structured `system_warnings` array and returns a `PARTIAL_SUCCESS` payload status to give API consumers transparent insight into downgraded or skipped operations.

<br>

## Support & Contributions
I built Lumina VE to provide developer teams with a fast, memory-safe tool for marketplace image compliance and web asset verification. If you discover a bug, want to discuss architectural enhancements, or need support patching edge cases, please feel free to contribute.
